...
const Cart = () => {
  ...
  useEffect(()=>{
    setTotalAmount(cart.reduce((acc,curr)=>acc+parseFloat(curr.price.split("$")[1]),0))
  }, [cart])
  ...
  <div>
      {
        ...
        (<div className="max-w-[1200px] mx-auto flex flex-col md:flex-row justify-center">
          ...
          <div className="w-[100%] md:w-[60%] flex flex-col p-2">
            {
              cart.map((item,index)=>(
                <CartItem key={item["isbn13"]} item={item} itemIndex={index}/>
              ))
            }
          </div>
        ...
        <div className="min-h-[80vh] flex flex-col items-center justify-center">
          ...
          <NavLink to={"/shopping"}>
            ...
            border-green-600 font-semibold hover:text-green-700 p-3 px-10 tracking-wider border-solid">
              Shop Now
            </button>
          </NavLink>
        </div>)
